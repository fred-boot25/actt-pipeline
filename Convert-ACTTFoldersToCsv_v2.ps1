...
function Convert-HeaderTokenToName {
    ...
    # Some ACTT header definitions wrap the column name itself in literal square brackets
    # (e.g. "[ACTTSystem] NVARCHAR(60)" in ACTT_Platform_MSSQL_All.actt). Strip those too,
    # so the resulting column name is "ACTTSystem", not "[ACTTSystem]" - otherwise every
    # downstream consumer that SQL-brackets identifiers ends up with "[[ACTTSystem]]".
    if ($name.StartsWith('[') -and $name.EndsWith(']') -and $name.Length -gt 2) {
